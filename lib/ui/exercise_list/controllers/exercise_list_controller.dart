...
class ExerciseListController extends GetxController
  ...
  getPlanName() {
    if (workoutPlanData != null) {
      if (workoutPlanData!.planDays == Constant.planDaysYes) {
        return "${"txtDay".tr} ${weeklyDaysData!.dayName}";
      } else {
  ...
  getWorkoutTime() {
    return "${workoutPlanData!.planMinutes ?? "0"} ${"txtMins".tr.toUpperCase()}";
  }
  ...
  getWorkoutLevel() {
    if (workoutPlanData!.planWorkouts == "0" &&
      ...
      return "${workoutPlanData!.planWorkouts!} ${"txtWorkouts".tr.toUpperCase()}";
    }
  }
