...
class PerformExerciseController extends FullLifeCycleController
  ...
  _argumentData() {
    if (arguments != null) {
      if (arguments[4] != null) {
        ...
        Debug.printLog("currentPos -->>$currentPos");
      }

      if (arguments[0] != null) {
        workoutPlanData = arguments[0];
        Debug.printLog(
            "workoutPlanData -->>${workoutPlanData!.toJson()}");
      }

      if (arguments[1] != null) {
        exerciseList = arguments[1];
        Debug.printLog("exerciseList -->>${exerciseList.toList()}");
      ...
      if (arguments[2] != null) {
        ...
        Debug.printLog(
            "listOfAnimation -->>${listOfAnimation.toList()}");
        setCurrentExe(currentPos);
      }

      if (arguments[3] != null) {
        listOfAnimationController = arguments[3];
        Debug.printLog("listOfAnimationController -->>${listOfAnimationController.toList()}");
        setCurrentExe(currentPos);
      }

      if (arguments[5] != null) {
        weeklyDaysData = arguments[5];
        Debug.printLog("weeklyDaysData -->>$weeklyDaysData");
        setCurrentExe(currentPos);
      }
    }
  }
  ...
  startPerformExercise() {
    ...
    if (!(currentExe!.exUnit == Constant.workoutTypeStep)) {
      ...
      Utils.textToSpeech(
          "${"tts3".tr} ${currentExe!.exTime} ${"tts4".tr} ${Utils.getMultiLanguageString(currentExe!.exName!)}",
          MyApp.flutterTts);
    }
  }
